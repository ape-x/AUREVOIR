...
u_int8_t expandedKey[10][KEY_SIZE];
...
void keySchedule(u_int8_t* key, int round){
    u_int8_t first = key[3];
    u_int8_t arr[4] = {key[3], key[7], key[11], key[15]};
    for(int i=0;i<3;i++){
        arr[i] = sBox[arr[i+1]];
    }
    arr[3] = sBox[first];
...
void keyExpansion(u_int8_t* key){
    u_int8_t k[KEY_SIZE];
    for(int i=0;i<16;i++){
        k[i] = key[i];
    }
...
void encryptBlock(u_int8_t* state, u_int8_t* encryption_key){
...
void decryptBlock(u_int8_t *state, u_int8_t* encryption_key){
    addRoundKey(state, &expandedKey[9]);
    for(int i = 8;i>=0;i--){
        invShiftRows(state);
        invSubBytes(state);
...
void encryptionECB(u_int8_t *text, u_int8_t* encryption_key, long bytes){
...
void decryptionECB(u_int8_t *text, u_int8_t* encryption_key, long bytes){
       long blocks = bytes/BLOCK_SIZE;
       keyExpansion(encryption_key);
...
fread(buff, 1, fileSize, handler); // reading file contents
fclose(handler);

if(!(handler = fopen("keyGenDump.txt", "r")))
    printf("Key file does not exist");

fread(key,1, KEY_SIZE, handler);

encryptionECB(buff, key, fileSize);

handler = fopen(argv[1], "w"); // Replacing with ciphertext
fwrite(buff, 1, fileSize, handler);
